#!/usr/bin/env bash
# =============================================================================
# ReachPilot Database Backup — Run INSIDE the LXC
# =============================================================================
...
BACKUP_DIR="/opt/reachpilot/backups"
TIMESTAMP=$(date '+%Y%m%d_%H%M%S')
BACKUP_FILE="reachpilot_${TIMESTAMP}.sql.gz"
...
for arg in "$@"; do
    case "$arg" in
        ...
        --install)
            ...
            LOG_PATH="/var/log/reachpilot-backup.log"
            CRON_LINE="0 2 * * * ${SCRIPT_PATH} --upload --cleanup >> ${LOG_PATH} 2>&1"
            (crontab -l 2>/dev/null | grep -v "reachpilot.*backup\|${SCRIPT_PATH}"; echo "$CRON_LINE") | crontab -
...
docker compose -f docker-compose.prod.yml exec -T db \
    pg_dump -U reachpilot -d reachpilot --no-owner --no-acl | gzip > "$BACKUP_PATH"
...
if [[ "$UPLOAD" == true ]]; then
    ...
    R2_BUCKET="${R2_BUCKET_NAME:-reachpilot-backups}"
...
if [[ "$CLEANUP" == true ]]; then
    ...
    while IFS= read -r f; do
        # Extract date from filename: reachpilot_YYYYMMDD_HHMMSS.sql.gz
        DAY=$(basename "$f" | sed 's/reachpilot_\([0-9]\{8\}\)_.*/\1/')
        if [[ -z "${SEEN_DAYS[$DAY]:-}" ]] && [[ ${#SEEN_DAYS[@]} -lt $KEEP_DAILY ]]; then
            KEEP_FILES["$f"]=1
            SEEN_DAYS["$DAY"]=1
        fi
    done < <(ls -t "${BACKUP_DIR}"/reachpilot_*.sql.gz 2>/dev/null)
    ...
    MONTH_COUNT=0
    while IFS= read -r f; do
        MONTH=$(basename "$f" | sed 's/reachpilot_\([0-9]\{6\}\).*/\1/')
