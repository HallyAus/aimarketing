...
export default async function UserDetailPage({
  ...
  const [user, auditLogs, posts, sessions] = await Promise.all([
  ...
  const sessionsTab = (
    <div style={sectionStyle}>
      {sessions.length === 0 ? (
        <p style={{ color: "var(--text-secondary)", fontSize: 14 }}>No active sessions.</p>
      ) : (
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          <thead>
            <tr>
          ...
          <tbody>
            {sessions.map((s) => {
              ...
              return (
                <tr key={s.id}>
                  <td style={{ ...tdStyle, fontFamily: "monospace", fontSize: 12 }}>{s.id.slice(0, 20)}...</td>
                  <td style={tdStyle}>{relativeTime(s.expires)}</td>
                  <td style={tdStyle}>
                    <span style={badgeStyle(isExpired ? "#e5484d" : "#30a46c")}>
